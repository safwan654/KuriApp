...
export default function KoottukuriJoinPage() {
  ...
  useEffect(() => {
    const performJoin = async () => {
      ...
      try {
        const result = await groupService.joinByCode(code, user.uid, profile.displayName);
        if (result) {
    ...
    return (
      <Shell>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-500 font-bold mb-8">Please login first to join this Group.</p>
      ...
      <div className="flex flex-col items-center justify-center py-32 text-center">
        {error ? (
          <div className="animate-up">
            ...
            <button onClick={() => router.push("/koottukuri")} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs">Back to List</button>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            <Loader2 size={48} className="text-purple-600 animate-spin mb-6" />
            <h1 className="text-2xl font-black mb-2">Joining Group... 🚀</h1>
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">Sathiye... Hold tight! 🔥</p>
          </div>
        )}
      </div>
    </Shell>
