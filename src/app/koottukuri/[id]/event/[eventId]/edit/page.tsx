...
export default function EditEventPage() {
  const { id, eventId } = useParams() as { id: string, eventId: string };
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [groupMembers, event] = await Promise.all([
          groupService.getGroupMembers(id),
          groupService.getEventDetails(id, eventId)
        ]);
        setFormData({
          title: event.title,
          hostName: event.hostName || "",
          date: event.date?.toDate?.().toISOString().split('T')[0] || event.date || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await groupService.updateEvent(id, eventId, formData);
          ...
          <form onSubmit={handleSubmit} className="p-10 md:p-14 space-y-10 bg-white">
            <div className="space-y-3">
              ...
              <input 
                type="text" 
                required
                className="w-full bg-white border-2 border-slate-100 rounded-2xl px-4 py-5 font-bold text-lg focus:border-purple-600 transition-all outline-none"
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <select 
                  className="w-full bg-white border-2 border-slate-100 rounded-2xl px-4 py-5 font-bold text-lg focus:border-purple-600 transition-all outline-none appearance-none cursor-pointer"
            ...
            <button 
              type="submit" 
              disabled={saving}
              className="w-full py-6 rounded-3xl font-black text-2xl text-white shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98] mt-4"
